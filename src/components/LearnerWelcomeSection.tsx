...
const LearnerWelcomeSection = ({ learnerName, surveyStatus }: LearnerWelcomeSectionProps) => {
  ...
  return (
    <Card className="mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-unboxable-navy/10">
      <CardHeader>
        <CardTitle className="flex items-center space-x-3 text-unboxable-navy">
          <GraduationCap className="h-6 w-6 text-unboxable-orange" />
          <span className="text-xl">{getWelcomeMessage()}</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              <h4 className="font-semibold text-unboxable-navy flex items-center">
                <ArrowRight className="h-4 w-4 mr-2 text-unboxable-orange" />
                Your Learning Path
              </h4>
              <ul className="space-y-2">
                {getInstructions().map((instruction, index) => (
                  <li key={index} className="flex items-start space-x-2 text-sm text-gray-700">
                    <div className="flex-shrink-0 w-5 h-5 rounded-full bg-unboxable-orange/20 flex items-center justify-center mt-0.5">
            ...
            <div className="bg-white/60 rounded-lg p-4 border border-white/40">
              <h4 className="font-semibold text-unboxable-navy flex items-center mb-3">
                <Users className="h-4 w-4 mr-2 text-unboxable-orange" />
                Support & Guidance
              </h4>
